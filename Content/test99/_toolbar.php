...
$filters=",";
$filenames=array();
$dir_handle=@opendir("../_pvt/filters") or exit("Unable to open $dirpath");
...
foreach($filenames as $filename){
	 ...
	 if(isImage($file) && !_in($file,"_tn.")){
		  	$url="../_pvt/filters/".$file;
		  	if($filters)$filters.=",";
		  	$filters.=$url;
}}
*/
...
//----- menus ------
var gMouseModeMenu=gMusicMenu=gAddMenu=gAddNewMenu=gAddChildMenu=gCopyMenu=gLinksMenu=gSearchMenu=gSwapMenu=null;

//@@@var gModeBtns=new Array('Image','Warp','Skew','Fold','Zoom','Slide','Spin','Tilt','Turn','Size','Move');
var gModeBtns=new Array('Image','Warp','Skew','Fold','Zoom','Slide','Size','Move','Turn','Spin','Tilt');
...
var gApiVid, gApiVidTitle, gApiId;

function api_playYoutubeVideo(vid,tit,id){
alert("api_playYoutubeVideo");
if(_in(tit,"<"))tit=tit.substring(0,_ix(tit,"<"));
gApiVid=vid;
gApiVidTitle=tit;
gApiId=id;
if(!_in(vid,"@vid@"))vid="@vid@"+vid;
loadVideo(gDir,vid,1,tit);
}


function api_saveItem(i,url,tit,id){
alert("api_saveItem")
if(_in(tit,"<"))tit=tit.substring(0,_ix(tit,"<"));
if(!gShownewvid){ alert("Rights violation"); return; }
if(i==null)i=gLastExpandedWindow;
var f=getWindowFrame(i);
if(i==2){
 if(!url && !f.ss && gApiVid){  //this needs to be changed (when loaded from chat it's NOT a slideshow so we have to check f.ss does NOT exist)
  url=gApiVid;
  tit=gApiVidTitle;
}}
if(!url){
...
if(i==2){  //videos
 asyncP("action=addvideo&tid="+url+"&tit="+tit+"&dir="+gDir,"api_"+i);
}}



function _api_savePic(src){
alert("_api_savePic")
if(_in(src,"@vid@")){
	 var a=src.split("&");
	 var tit="",vid=a[0];
	 ...
	 if(_in(tit,"<"))tit=tit.substring(0,_ix(tit,"<"));
	 alert(tit);
	 asyncP("action=addvideo&tid="+vid+"&tit="+tit+"&dir="+gDir,"savevid");
}else{
	 openPopup("newimage","_newutil.php?page=newfimg&dir="+gDir+"&src="+src.replace("http://","http_"),"New Image Link");
}}


function showSSthumbs(i){
alert("showSSthumbs")
getWindowFrame(i).ss.flipThumbs();
}
...
if(this.nbrname)this.nbrname++;
else this.nbrname=_rdm(1,99999);
var name=this.nbrname;
...
var url="_takePicFrame.php?name="+name+"&dir="+gDir+"&play="+play;
openPopup("takepic",url,'<big><big>Capturing Image: '+name+'.png</big></big>');
}
...
function saveRekord(){
openPopup("saverekord","_newutil.php?page=saverekord&dir="+gDir+"&name="+gPixi.gPic.aniName,"Save Action");
}

function saveAutoplay(){
alert("not done yet");
//openWindow("saveautoplay","_newutil.php?page=saverekord&dir="+gDir+"&name="+gPixi.gPic.aniName,"Save Action");
}
...
for($tmp=0;$tmp<4;$tmp++){
	 ...
	 wrt("<td width=34><IMG id=iWinHide".$tmp." onclick='hideWindow(".$tmp.")' src='_pvt_images/folders/toggle_minimize.png' style='display:none;width:28px;cursor:pointer;position:relative;top:0px;left:0px;'></td>".
	     "<td width=34><IMG id=iWinToggle".$tmp." onclick='expandWindow(".$tmp.")' src='_pvt_images/folders/toggle_expand.png' style='display:none;width:28px;cursor:pointer;position:relative;top:0px;left:0px;'></td>".
	 	 "<td width=34><IMG onclick='closeWindow(".$tmp.")' src='_pvt_images/delete.png' style='width:26px;cursor:pointer;position:relative;top:0px;left:0px;'></td>".
	     "</tr></TABLE>".
	     "</td></tr><tr>".
...
<TABLE id=iMainTable border=0 cellpadding=0 cellspacing=0 width=100% height=100% style='position:absolute;top:0px;left:0px;width:100%;height:100%;overflow:hidden;'>
 ...
 <td align=left colspan=2 style='border-bottom:solid 1px #ccc; height:28px;overflow:hidden;'>
	 ...
	 wrt("<td style='width:190px;height:28px;border-right:solid 0px #ccc;'>");
	 
		//<!-- ============================ ADMIN BUTTONS =================================== -->
		wrt("<TABLE id=iAdminButtons border=0 cellpadding=0 cellspacing=0 style='display:block;overflow:hidden;'><tr>");
			wrt("<td><div id=iToolbarSpacer style='width:41px;'></div></td>");
			wrt("<td>");
	 ...
	 wrt("</td>");
	 // ==================== MOUSE MODEBUTTONS =======================
	 wrt("<td align=right style='width:200px;'>");
	 ...
	 ?>
		...
		<DIV id=iBoxBdrTop class='corners4' style='overflow:hidden;height:28px;color:black;'>
			...
			<td align=left style=''>
				...
				<td><img class='c_barbtn corners4' src='_pvt_images/mouse.png' onclick='gMouseModeMenu.clickMenuBtn(this)' style='width:20px;height:20px;' ></td>
				...
				<td style='padding-left:4px;'><img id='iAutoFwd' class='c_barbtn corners4' onmousedown='gPixi.gPic.screen.animation.startstop(1)' src='_pvt_images/play.png' oncontextmenu='return false;' title='play' style='opacity:0.7;position:relative;left:0px;top:0px;'></td>
				</tr>
				</TABLE>
			</td>
			<td width=4></td>
...
//wrt("<td>&nbsp;</td><td>");
//if($showfolders)wrt("<IMG id=iFldrBtn class='c_barbtn corners4' src='_pvt_images/folders-stack.png' onclick='fmtMenu(\"all\")' style='height:20px;width:20px;padding-left:12px;' title='show all'>");
//if($showpics)wrt("<IMG id=iPicsBtn class='c_barbtn corners4' src='_pvt_images/pics.png' onclick='schedFmtMenu(\"pics\")' ondblclick='loadSSall(\"images\",\"ssImages\")' style='float:left; height:20px;width:20px;padding-left:8px;' title='show images'>");
//if($showvids)wrt("<IMG id=iVidsBtn class='c_barbtn corners4' src='_pvt_images/videos.png' onclick='schedFmtMenu(\"vids\")' ondblclick='loadSSall(\"videos\",\"ssVideos\")' style='float:left; height:20px;width:20px;padding-left:12px;' title='show videos'>");
//wrt("<IMG id=iFilsBtn class='c_barbtn corners4' src='_pvt_images/new_page.png' onclick='schedFmtMenu(\"files\")' ondblclick='loadSSall(\"files\",\"ssImages\")' style='float:left; position:relative; top:2px; height:20px;width:20px;padding-left:12px;' title='show files'>");
//wrt("<IMG id=iLnksBtn class='c_barbtn corners4' src='_pvt_images/link.png' onclick='schedFmtMenu(\"links\")' ondblclick='loadSSall(\"links\",\"ssImages\")' style='float:left; position:relative; top:2px; height:20px;width:20px;padding-left:12px;' title='show links'>");
//wrt("<IMG id=iPixiBtn class='c_barbtn corners4' src='_pvt_images/picxie.png' onclick='schedFmtMenu(\"pixis\")' style='float:left; position:relative; top:2px; height:20px;width:20px;padding-left:12px;' title='show pixis'>");
//wrt("</td>");

//if($showsearchbtn=="yes")wrt("<td id=iSearchBtn >&nbsp;<IMG class=c_barbtn src='_pvt_images/duckduck_logo.png' onclick='pageLoader(\"search\")' title='search the web' style='width:22px;height:19px;'></td>");
//wrt("<td>&nbsp;<IMG id=iLoginInfoBtn class=c_barbtn src='_pvt_images/help.png' onclick='welcomeMsg()' title='help' style='width:20px;height:20px;'></td>");
//wrt("<td><img class='c_barbtn corners4' id=iAnimateBtn onclick='gPixi.gPic.screen.animation.startstop()' src='_pvt_images/play.png' title='animate' style='position:relative;left:-4px;'></td>");
//wrt("<td><img id=iRecordBtn class='c_barbtn corners4' src='_pvt_images/record.png' onmousedown='gPixi.gPic.screen.rekord.record(event,this)' oncontextmenu='return false;' title='record actions'></td>");
//wrt("<td><img id='iStartStopPlay' class='c_barbtn corners4' onmousedown='gPixi.gPic.screen.rekord.startStopPlaying(null,event)' src='_pvt_images/play.png' oncontextmenu='return false;' title='animate' style='display:none; position:relative;left:-4px;'></td>");
//if($gLoggedIn)wrt("<td width=31><img id='iSaveConfigBtn' class='c_barbtn corners4' src='_pvt_images/save1.png' onclick='savePixi()' title='save view'  style='width:28px;height:26px;position:relative;left:2px;' ></td>");
//wrt("<td><img class='c_barbtn corners4' onclick='gPixi.gPic.reset();gPixi.gToolbar.syncControls();' style='width:20px;height:20px;' src='_pvt_images/reset.png' title='reset'></td>");
/*
	wrt("<TABLE id=iWinBtnsX class='slideOpacity' cellpadding=0 cellspacing=0 style='display:;'><tr>".
    	"<td width=34><IMG id=iWinSaveX onclick='api_saveItem()' src='_pvt_images/favoriteadd.png' style=' DISPLAY:NONE; width:24px;cursor:pointer;position:relative;top:-2px;left:0px;' title='save'></td>".
    	"<td width=34><IMG id=iWinHideX onclick='hideWindow()' src='_pvt_images/folders/toggle_minimize.png' style=' DISPLAY:NONE; width:26px;cursor:pointer;position:relative;top:0px;left:0px;padding-left:0px;'></td>".
    	"<td width=34><IMG id=iWinToggleX onclick='expandWindow()' src='_pvt_images/folders/toggle_collapse.png' style=' width:22px;cursor:pointer;position:relative;top:0px;left:0px;padding-left:8px;'></td>".
    	"<td width=34><IMG onclick='closeWindow()' src='_pvt_images/delete.png' style=' DISPLAY:; width:24px;cursor:pointer;position:relative;top:0px;left:0px;padding-left:0px;padding-right:20px;'></td>".
    	"</tr></TABLE>");
	wrt("</td><td>");
*/
?>
