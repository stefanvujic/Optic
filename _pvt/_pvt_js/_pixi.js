...
- put some javascript files at the end of the page? (just before </body>)
- combine some .js files (and .php?) into one (just for pixi.php)
- can we call parent.functions() instead so we don't need some files at all??

- add keys in 'embed' for playing slideshow and for next image
...
//x+=" gPic.docWidth="+gPic.docWidth+"; ";
//x+=" gPic.docHeight="+gPic.docHeight+"; ";
...
function History(scr){
	...
	this.curve=50;
	this.smoothed=0;
	this.relative=1;		//works but really only applies to saved actions so we comment it out
	this.mergeHistory=1;
	this.timeout=10;
	this.delta=25;
	this.IX=-1;
...
if(this.loopmode=="stop"){ this.IX = (arrow==1)? 0 : this.HISTORY.length-1; }
this.resmooth=0;
...
var lastix=ITMS[0];
if(lastix==cmdix)cmdix=1;		// some bug causes this
ITMS[0]=cmdix;
...
if(_in(cmd,".")){							// it's not a '.SV' command
	...
	//--- if relative ----
	//msg("newx 1="+newx);
	if(this.relative && s.stdVar(cmd)){
		var oldx=this.getSplitVal(lastVal,0);
		var oldy=this.getSplitVal(lastVal,1);
		var dx=newx-oldx;
		var dy=newy-oldy;
		newx=s.getVar(cmd,0,oldx)+dx; 
		newy=s.getVar(cmd,1,oldy)+dy;
	}
	//msg("newx 2="+newx);
	//msg("-------------------");
	if(once || !this.smoothing){
		...
		gPic.unDoing=1;
		//s.SV(cmd, newx,newy);
		//gPic.repaint(this.scrix); //!!!!!
		gPic.applyVars(s,cmd,newx,newy);
		scr.undoing=0;
		gPic.unDoing=0;
	}else{
		//================= play a smoothed path ===================
		//msg("cmdix="+cmdix+", "+lastVal+" to "+newVal);
		...
		if(lastix==newix)lastix=1;	// some bug causes this!
		for(var i=1; i<a.length; i++){ 		// get the LAST newVal
			if(a[i]==newVal)newix=i;
		}
		//if(lastix==newix)newix+=arrow;
		//if(newix>a.length-1){  msg("NEWIX OUT OF RANGE: "+newix); }
...
//msg("lastix="+lastix);
//msg("newix= "+ix);
//msg("targetix="+targetix);
//if(lastix==targetix){		msg("lastix==targetix="+lastix); 	return; 	}		// path has completed
//if(ix<1 || ix>a.length-1){		msg("ix OUT OF RANGE ix="+ix);	return;		}
if(ix<1 || ix>a.length-1){		msg("ix OUT OF RANGE ix="+ix+" (setting to 1)"); 		ix=1;		}
var newVal=a[ix];
var newx=this.getSplitVal(newVal,0);
var newy=this.getSplitVal(newVal,1);
//--- if relative ----
if(this.relative && s.stdVar(cmd)){
	var lastVal=a[lastix];
	var oldx=this.getSplitVal(lastVal,0);
	var oldy=this.getSplitVal(lastVal,1);
	var dx=newx-oldx;
	var dy=newy-oldy;
...
scr.undoing=1;
gPic.unDoing=1;
//scr.SV(cmd, newx,newy);
//gPic.repaint(scr.scrix);
...
//--- option - oscillate or restart?---- (NOTE: All itms[] should have the same length)
//msg("a="+a);
//msg("1 ix="+ix);
//msg("1 itms="+itms);
...
for(var i=1;i<a.length-1;i++){
	...
	if(_in(cmd,".")){ //--- non SV ---
		...
		if(this.relative && s.stdVar(cmd)){
			var lastVal=itms[lastix];
			var oldx=this.getSplitVal(lastVal,0)*1;
			var oldy=this.getSplitVal(lastVal,1)*1;
			var dx=newx-oldx;
			var dy=newy-oldy;
			newx=(s.getVar(cmd,0,oldx)*1)+dx; 
			newy=(s.getVar(cmd,1,oldy)*1)+dy;
		}
		//scr.SV(cmd, newx,newy);	//!!!!!
		//msg("apply="+cmd+"("+newx+","+newy+")");	//!!!! this applies the same values over and over
...
//msg("------------------------------");
	//msg("REDONE arrow="+this.arrow+", IX="+this.ix+", V="+action);
}
...
//----------------------- unredo -------------------------
History.prototype.unredo=function(arrow){
if(this.loopmode=="oscillate")arrow=this.direction*arrow;
var scr=s=this.screen;
if(this.HISTORY.length<1)return;
var a=this.HISTORY;
//--- set IX ---
if(this.arrow!=arrow)this.arrow=arrow;  //direction chg
else 	this.IX=this.chgix(this.HISTORY,this.IX,arrow);
var cmd=this.HISTORY[this.IX];
//--- check there is more than one value...
var originalix=this.IX;
if(this.ITEMS[cmd].length<3){
    this.IX=this.chgix(this.HISTORY,this.IX,arrow);
	while(this.ITEMS[cmd].length<3 && originalix!=this.IX){
        this.IX=this.chgix(this.HISTORY,this.IX,arrow);
		var cmd=this.HISTORY[this.IX];
	}
}
if(this.ITEMS[cmd].length<3)return;		// nothing to do
var a=this.ITEMS[cmd];
var i=this.chgix(a,a[0],arrow,1);
this.ITEMS[cmd][0]=i;
var newix=i;
scr.undoing=1;								// if undoing then we don't record the actions
gPic.unDoing=1;
if(_in(cmd,".")){							// it's not a '.SV' command
	var action=cmd+"("+a[newix]+");";
	myEval(action);				//repaint?   //!!!!! redo and unredo seem to work ok
	gPic.repaint(scr.scrix);  	//be nice to use applyVars but not sure what is recorded here
}else{
	//s.SV(cmd, this.getSplitVal(a[newix],0),this.getSplitVal(a[newix],1)  );
	gPic.applyVars(s,cmd,this.getSplitVal(a[newix],0),this.getSplitVal(a[newix],1));
}
//gPic.repaint(scr.scrix);
gToolbar.syncControls();
scr.undoing=0;
...
History.prototype.chgix=function(a,ix,arrow,lower){
if(!lower)lower=0;
ix+=arrow;
if(ix<lower)ix=a.length-1;
if(ix>(a.length-1))ix=lower;
return ix;
}
...
History.prototype.setRekRelative=function(v){
this.relative=(v)?1:0;
}
	...
	//msg("2. speedx="+spdx+", speedy="+spdy);
 	switch(gPic.xmode){
		...
		case "xZoom"	:
				 ...
				 break;
		// adjust the following for actual slider values? 
	  	case "xSize"  : 
			...
			break;
	  	case "xTurn"  : 
			this.anix=scr.vx;  this.aniy=scr.vy;      
			if(this.xrange_max>96)this.xrange_max=96; 
			if(this.yrange_max>96)this.yrange_max=96; 
			this.timeout=300;  
			break;
	  	case "xRotate": 
		...
		case "xTilt"  : 
	  	...
	  	case "xSlide"  :
	   		this.anix=scr.xSlide;
	   		this.aniy=scr.ySlide;
	   		break;
	 }
	if(scr.history.loopmode=="stop"){
		if(this.aniXon && this.anix>=(this.xrange_max-this.speedx))this.anix=this.xrange_min;
		if(this.aniYon && this.aniy>=(this.yrange_max-this.speedy))this.aniy=this.yrange_min;
	}
 	this.animate(1);
 	this.flashOn=0;
 	this.playBlinker();
...
if(firsttime){
	...
	if(this.anixStart==this.anix)this.xLoop++;
	if(this.aniyStart==this.aniy)this.yLoop++;
	if( (this.aniXon && this.xLoop>1) || (this.aniYon && this.yLoop>1)){
		scr.undoing=1;
		gPic.unDoing=1;
		//msg("STOP recording.... anix="+this.anix+", aniy="+this.aniy);
	}
}
		...
		case "x"	:
			if(v<=this.xrange_min)  {
				switch(scr.history.loopmode){
					...
					case "restart": 		return this.xrange_max;
					case "stop":			this.stop();		return this.xrange_min;
				}
			}
			if(v>=this.xrange_max)  {
				//msg("d="+d+", v="+v+", speedx="+this.speedx+", min="+this.xrange_min+", max="+this.xrange_max+", loopmode="+scr.history.loopmode);
				switch(scr.history.loopmode){
					case "oscillate":	this.speedx*=-1;		return this.xrange_max;
					case "restart": 		return this.xrange_min;
		...
		case "y"	:
			if(v<=this.yrange_min)  {
				switch(scr.history.loopmode){
					...
					case "restart": 		return this.yrange_max;
					case "stop":			this.stop();		return this.xrange_min;
				}
			}
			if(v>=this.yrange_max)  {
				switch(scr.history.loopmode){
					...
					case "restart": 		return this.yrange_min;
					case "stop":			this.stop();		return this.xrange_max;
				}
			}
			break;
