...
- animations?
	- replay : smooth, etc?
	- replay action : 'load', 'edit', 'reset'  ?  (and also autoplays once savable?)
	- autoplay : relative for all modes?
	- autoplay : test all mousemodes
	- autoplay : save?

- after AutoPlay UNDO doesn't seem to work?
...
function History(scr){
	...
	this.smoothing=0;
	this.curve=50;
	this.relative=0;		//works but really only applies to saved actions so we comment it out
	this.mergeHistory=0;
	this.timeout=10;
	this.delta=25;
	this.IX=-1;
 	...
 	switch(gPic.xmode){
		...
		case "xZoom"	:
	  	case "xSlide"   :
				...
				//msg("relative="+this.relative);
				//if(this.relative){
					this.xrange_min=this.relativeAdjust(this.xrange_min,this.anix,50,2,98);
					this.xrange_max=this.relativeAdjust(this.xrange_max,this.anix,50,2,98);
					this.yrange_min=this.relativeAdjust(this.yrange_min,this.aniy,50,2,98);
					this.yrange_max=this.relativeAdjust(this.yrange_max,this.aniy,50,2,98);
				//}
				break;
