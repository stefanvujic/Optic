...
$u="_pvt/_toolbarNEW.php".$parms;

//------ look for published URL ------
/* ????
//if($subdomains)exit("subdomains=".$subdomains);
if(!$subdomains)$subdomains=$_GET['subdomains'];
if($subdomains){
 //$url=myReadFile("_pvt/subdomains/".$subdomains.".txt");
 $a=_split($subdomains,".");
 $f=($a[1])? $a[1]."/"."_pvt_pub/".$a[0].".txt" :  "_pvt_pub/".$a[0].".txt" ;
 $url=myReadFile($f);
 $u=$url;
 //exit("url=".$url.", f=".$f);
}
...
//<!DOCTYPE html>   this fucks up the doc height!!
?>
<!DOCTYPE html>
<HTML lang="en">
<head>
